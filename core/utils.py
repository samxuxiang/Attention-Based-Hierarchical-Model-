...
def load_coco_data(data_path='./data', split='train'):
    ...
    data['features1'] = hickle.load(os.path.join(data_path, '%s.pool5.features.hkl' %split))
    data['features2'] = hickle.load(os.path.join(data_path, '%s.pool4.features.hkl' %split))
...
def sample_coco_minibatch(data,batch_size):
    data_size = data['features1'].shape[0]
    mask = np.random.choice(data_size, batch_size)
    features1 = data['features1'][mask]
    features2 = data['features2'][mask]
    file_names = data['file_names'][mask]
    return features1,features2, file_names
